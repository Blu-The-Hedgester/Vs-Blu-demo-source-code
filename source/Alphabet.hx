...
class Alphabet extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		if (isMenuItemCenter)
		{
			...
			var lerpVal:Float = CoolUtil.boundTo(elapsed * 9.6, 0, 10);
			y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.48) + yAdd, lerpVal*4);
		...
		if (isMenuItem)
		{
			...
			if(forceX != Math.NEGATIVE_INFINITY) {
				...
				x = FlxMath.lerp(x, (targetY * 20) + 90 + xAdd, lerpVal*2);
			}
		}
