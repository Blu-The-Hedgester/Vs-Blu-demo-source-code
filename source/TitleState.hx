...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var background:FlxSprite = new FlxSprite().loadGraphic(Paths.image('titleBG'));
		...
		if (animFrames.length > 0) {
			...
			titleText.animation.addByPrefix('idle', "ENTER IDLE", 3);
			titleText.animation.addByPrefix('press', ClientPrefs.flashing ? "ENTER PRESSED" : "ENTER FREEZE", 5);
		}
		else {
			newTitle = false;
			
			titleText.animation.addByPrefix('idle', "Press Enter to Begin", 3);
			titleText.animation.addByPrefix('press', "ENTER FREEZE", 5);
		}
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (mustUpdate) {
						...
						//MusicBeatState.switchState(new MainMenuState());
						MusicBeatState.switchState(new ImportantNoteState());
					}
					closedState = true;
				});
